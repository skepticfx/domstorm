...
var getBrowserResults = function(module) {
  ...
  for (var x in browsers) {
    ...
    if (browser_temp.rows && browser_temp.rows.length > 0) {
      ...
      var table_html = '<br/><div class="table-responsive">'+ is_last_run +'<div class="label label-danger">Tested on</div><div class="label label-info">' + browser_temp.name + ' - ' + browser_temp.version + ' - ' + browser_temp.os + '</div><table class="table table-striped table-bordered table-hover"> <thead><tr class="TITLE">';
      // Iterate the columns
